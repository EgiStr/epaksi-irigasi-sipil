...
export function determineQualityClass(score, grading) {
  // Support both old A/B/C/D and new BAIK_SEKALI/BAIK/KURANG/JELEK grading
  const gradeKeys = grading ? Object.keys(grading) : [];
  
  // If grading config exists, use it in order of decreasing min values
  if (grading && gradeKeys.length > 0) {
    const sortedGrades = gradeKeys
      .map(key => ({ key, ...grading[key] }))
      .sort((a, b) => (b.min || 0) - (a.min || 0));
    
    for (const gradeInfo of sortedGrades) {
      if (score >= (gradeInfo.min || 0)) {
        return {
          grade: gradeInfo.key,
          label: gradeInfo.label,
          color: gradeInfo.color,
          range: `${gradeInfo.min || 0}-${gradeInfo.max || 100}`
        };
      }
    }
  }
  
  // Fallback to default grading if no config provided
  if (score >= 85) {
    return { grade: 'BAIK_SEKALI', label: 'Baik Sekali', color: '#22c55e', range: '85-100' };
  } else if (score >= 70) {
    return { grade: 'BAIK', label: 'Baik', color: '#eab308', range: '70-84.99' };
  } else if (score >= 55) {
    return { grade: 'KURANG', label: 'Kurang', color: '#f97316', range: '55-69.99' };
  } else {
    return { grade: 'JELEK', label: 'Jelek', color: '#ef4444', range: '0-54.99' };
  }
}
